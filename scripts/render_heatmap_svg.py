...
GitHub-style contribution heatmap SVG in a customized zinc-blue colorway.
...
OUT_PATH = os.path.join(HERE, "..", "sanyam-heatmap.svg")

# Sanyam's clean blue ramp matching his backend/infra aesthetic
PALETTE = ["#18181b", "#172554", "#1e3a8a", "#2563eb", "#3b82f6", "#60a5fa"]
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">sanyam@github: ~/contributions --graph</text>')
...
if __name__ == "__main__":
    try:
        ...
        import sys
        print(f"Error rendering heatmap: {e}", file=sys.stderr)
        sys.exit(1)
